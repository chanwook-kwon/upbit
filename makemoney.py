...
ADA_OneStep_Price = 1
ADA_OneStep_Percent = 0.39
ADA_Buy_Step = 2 #2*0.39% = 0.78% 현재 가격의 0.78%에 매수함.
ADA_Sell_Step = 4 #4*0.39 = 0.54% - 현재 가격의 0.54%에 매도함.
...
while True:
    try:
        ...
        if (ADA_Balance > 0) and ((ADA_Average_Ma30 + ADA_GAP == ADA_Average_Ma60)) : #보유코인이 있으면 매도한다.(이익일수도 있고, 손절일수도 있고)
            ADA_ret_Sell = upbit.sell_market_order(Coin_ADA,ADA_Balance)
            print("매도성공",ADA_ret_Sell)
            ADA_Sell_Count = ADA_Sell_Count + 1
        if (ADA_Balance == 0) and ((ADA_Average_Ma30 == ADA_Average_Ma60 + ADA_GAP)) : #보유코인이 없으면 -> 매수 진행.
            ...
            if ADA_Average_Ma30 > ADA_Average_Ma60 :
                print("매수가능 GAP=",(ADA_Average_Ma60-ADA_Average_Ma30))
            else :
                print("매도가능 GAP=",(ADA_Average_Ma30-ADA_Average_Ma60))
